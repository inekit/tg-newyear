...
class Step {
  constructor(obj) {
    if (typeof obj === "function")
      return Object.assign(this, { type: "handler", handler_template: obj });
    ...
    Object.assign(this, {
      ...
      keyboard: skipTo
        ? {
            ...
            args: [[skipText ?? "BUTTON_SKIP"], ["skip"]],
          }
        : keyboard,
      type: type ?? "input",
...
class Steps {
  ...
  getStepIdByVariable = (variable) =>
    this.stepsArray.findIndex((el, i) =>
      el.variable === variable ? el : undefined
    );
  ...
  previousByVariable = (variable) =>
    this.stepsArray.find((el, i) =>
      el.variable === variable ? this.stepsArray[i - 1] : false
    );
}
...
function selectKeyboardOpt(soArray, prefix) {
  ...
  if (soArray[0] && Array.isArray(soArray[0])) {
    if (
      !soArray[1] ||
      !Array.isArray(soArray[1]) ||
      soArray[0].length !== soArray[1].length
    )
      throw new Error("sel opt is wrong");
    if (
      soArray[0].find((so) => typeof so !== "string") ||
      soArray[1].find((so) => typeof so !== "string")
    )
      throw new Error("all sel opt should be a string");
    else return soArray;
  } else if (soArray.find((so) => typeof so !== "string"))
    throw new Error("all sel opt should be a string");
  else {
    if (!prefix || typeof prefix !== "string") prefix = "";
    return [
      soArray.map((so) => prefix.toUpperCase() + so.toUpperCase()),
      soArray,
    ];
  }
}
...
function formatMenuOptions(soArray, sceneName) {
  if (!(typeof soArray === "object" && soArray !== null))
    throw new Error("menu opt should be an obj");
  "ff".concat("_BUTTON");
  if (Array.isArray(soArray))
    return soArray.reduce((prev, cur, i) => {
      cur = cur.toString();

      const formattedSN = sceneName
        ? `${sceneName
            ?.replace(/([A-Z])/g, "_$1")
            ?.trim()
            ?.toUpperCase()}_BUTTON_`
        : "BUTTON_";
      const autoNaming =
        formattedSN +
        cur
...
function createH(type, stepInfo, nextStepInfo, scene) {
    ...
    if (stepInfo.options && !stepInfo.cb)
      handler.action(stepInfo.options, (ctx) =>
        actions.addInput(
          ...
          nextStepInfo?.header,
          null,
          nextStepInfo?.keyboard
        )
      );
    else if (stepInfo.options && stepInfo.cb)
      handler.action(stepInfo.options, (ctx) => stepInfo?.cb(ctx));
  ...
  if (type === "menu") {
    ...
    if (stepInfo.options && stepInfo.cb)
      handler.hears(optionsTexts, async (ctx) => {
      ...
      handler.hears(Object.keys(stepInfo.options), (ctx) => {
        ctx.scene
          .enter(stepInfo.options[ctx.message?.text])
          .catch((e) =>
            ctx.replyWithTitle(
              `Нет такой сцены ${stepInfo.options[ctx.message?.text]}`
            )
          );
      });
    else throw new Error("no opt or cb");
  ...
  if (type === "action") {
    ...
    if (!stepInfo.cb)
      handler.on("text", async (ctx) =>
        replyTemplates.addSceneInput(ctx, {
          stepName: stepInfo.variable,
          header:
            nextStepInfo?.header ??
            `ENTER_${nextStepInfo?.variable?.toUpperCase()}`,
    ...
    if (type === "confirm" && stepInfo?.cb)
      handler.action("confirm", async (ctx) => {
        stepInfo?.cb(ctx);
      });
    else if (type === "confirm_next" || type === "confirm")
      handler.action("confirm", async (ctx) => {
        ...
        if (status) {
          if (scene !== "ordersScene")
            return await ctx.scene.enter("alpinistScene");
    ...
    if (type === "confirm_next") {
      if (!stepInfo?.handler_template)
        handler.on("text", async (ctx) =>
          replyTemplates.addSceneInput(ctx, {
            stepName: stepInfo.variable,
            header:
              stepInfo?.confirm_header ??
              `ENTER_${nextStepInfo?.variable?.toUpperCase()}`,
            kbName: {
              name: "custom_keyboard",
              args: [
                ["BUTTON_ENOUGH", nextStepInfo?.next ?? "Продолжить"],
                ["confirm", "next"],
              ],
            },
            confineNames: stepInfo.confines,
      ...
      handler.action("next", async (ctx) => {
        ...
        return replyTemplates.startDialog(
          ctx,
          nextStepInfo?.header ?? ".",
          nextStepInfo?.keyboard
        );
