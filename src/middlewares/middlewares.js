...
module.exports = {
    deleteMessage(ctx,pm1,pm2,pm3){
        setTimeout(()=>{
            if (ctx.scene.state.pm1) {ctx.tg.deleteMessage(ctx.chat.id, ctx.scene.state.pm1); }
            ctx.scene.state.pm1 = pm1;
            if (ctx.scene.state.pm2) {ctx.tg.deleteMessage(ctx.chat.id, ctx.scene.state.pm2); }
            ctx.scene.state.pm2 = pm2;
            if (ctx.scene.state.pm3) {ctx.tg.deleteMessage(ctx.chat.id, ctx.scene.state.pm3); }
    ...
    initKeyboards(kbName){
        ...
        if (kbName === "skip_keyboard" || kbName === "skip_previous_keyboard") kbBottom = kbName
        ...
        else if (kbName === "confirm_keyboard" || 
        ...
        kbTop = kbTop ?? (kbName === "skip_previous_keyboard" ? "skip_previous_keyboard" : "previous_step_keyboard")

        if (kbName === "coordinates" || kbName === "skip_previous_keyboard") kbTop = null;
    ...
    async sendInputReply(ctx, header, kbTop, kbBottom, showInputs = false){
        ...
        if (ctx.scene?.state?.input)  paramsString = Object.entries(ctx.scene?.state?.input)?.filter(([name,value])=>{

            return name!="scan" && name!="clientId" && name!="client_id" && name!="photos" && 
             name!="audio" && name!="position" && name!="audios" && name!="documents"
        }
        ).map(([name,value])=>{
            if (Array.isArray(value)) return ctx.getTitle(name) + ": " + ctx.getTitle(value.join(', '))
