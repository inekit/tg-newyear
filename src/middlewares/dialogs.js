const ConfineChecker = require("../Modules/confines");
const {initKeyboards, sendInputReply, deleteMessage} = require("./middlewares")
...
module.exports= {
    ...
    async addSceneInput(ctx, {stepName, header, confineNames, kbName, isLast, sceneName}){
        ...
        if (!ConfineChecker.passedConfines(ctx?.message?.text,confineNames, stepName)) return ctx.replyWithTitle(ctx.scene.state.previousInput ?? "TRY_AGAIN")
