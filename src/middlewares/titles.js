...
class Titles {
  constructor(dataFolder = "./Titles") {
    ...
    for (let lang of langs) {
      ...
      let obj = JSON?.parse(
        fs.readFileSync(`${this.dataFolder}/ru/titles.json`, "utf-8")
      );
      Object.keys(obj).map((key) =>
        this.dictionary[key]
          ? (this.dictionary[key][lang] = obj[key])
          : (this.dictionary[key] = { [lang]: obj[key] })
      );
    }
  }
  ...
  insert(string, language, replacers, markup) {
    const generator = function* (replacers) {
      for (const replacer of replacers) {
        yield this.dictionary[replacer]
          ? this.dictionary[replacer][language] || this.dictionary[replacer]
          : replacer;
      }
    }.bind(this)(replacers);
  ...
  getTitle(title, language, replacers, markup) {
    if (!this.dictionary[title]) {
      if (title?.toUpperCase() === title) {
        ...
        file[title] = title;
        fs.writeFileSync(
          `${this.dataFolder}/ru/titles.json`,
    ...
    let res = replacers
      ? this.insert(string, language, replacers, markup)
      : string;
