...
module.exports = ({
	...
	replyWithCopy: async function(keyboard, section, language) { 
		...
		if(!keyboard) return this.telegram.copyMessage(this.from.id, chat_id, message_id).catch(e => {})

		const extra = createKeyboard(keyboard, this)

		try {
			
			const response = await this.telegram.copyMessage(this.from.id, chat_id, message_id, extra)
	...
	sendTitleOpt: async function(id, title, language, options) { 
		...
		return this.telegram.sendMessage(id, this.getTitleOpt(title, opt_lang, options), { parse_mode: 'HTML' }).catch(e => {})
	},
